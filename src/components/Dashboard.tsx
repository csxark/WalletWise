...
const Dashboard = () => {
        ...
        <div className="flex items-center space-x-3 sm:space-x-9">
          ...
          <div className="relative">
            <button
              ...
              className={`flex items-center space-x-2 border rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 transition-colors shadow-sm text-xs sm:text-sm lg:text-base ${
                isDark
  ...
  `}>
                {availableMonths.length > 0 ? (
                  availableMonths.map((month) => (
                    <button
                      ...
                      className={`w-full text-left px-3 sm:px-4 py-2 sm:py-3 transition-colors first:rounded-t-lg first:sm:rounded-t-xl last:rounded-b-lg last:sm:rounded-b-xl text-xs sm:text-sm ${
                        selectedMonth === month 
                          ? isDark
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
        <div className={`rounded-xl sm:rounded-2xl shadow-xl border p-3 sm:p-4 lg:p-6 hover:shadow-2xl transition-all duration-300 hover:scale-105 backdrop-blur-sm ${
          isDark
        ...
        <div className={`rounded-xl sm:rounded-2xl shadow-xl border p-3 sm:p-4 lg:p-6 hover:shadow-2xl transition-all duration-300 hover:scale-105 backdrop-blur-sm ${
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-sm sm:text-lg lg:text-2xl font-bold mt-1 sm:mt-2 ${
                savingsRate >= 20 
            ...
            <div className={`h-8 w-8 sm:h-10 sm:w-10 lg:h-12 lg:w-12 rounded-lg sm:rounded-xl flex items-center justify-center shadow-lg ${
              isDark
        ...
        <div className={`col-span-2 lg:col-span-1 rounded-xl sm:rounded-2xl shadow-xl border p-3 sm:p-4 lg:p-6 hover:shadow-2xl transition-all duration-300 hover:scale-105 backdrop-blur-sm ${
          isDark
            ? 'bg-slate-800 border-slate-700'
            : 'bg-white border-gray-100'
        }`}>
      ...
      <div className={`rounded-xl sm:rounded-2xl shadow-xl border p-4 sm:p-6 lg:p-8 backdrop-blur-sm ${
        isDark
          ? 'bg-slate-800 border-slate-700'
          : 'bg-white border-gray-100'
      }`}>
          ...
          <div className="h-48 sm:h-64 lg:h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyData}>
                ...
                <XAxis dataKey="month" stroke={isDark ? '#94a3b8' : '#64748b'} fontSize={10} />
            ...
            {categoryData.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full text-gray-500 dark:text-gray-400">
                <div className="text-center">
                  ...
                  <p className="font-medium text-sm sm:text-base">No expense data for {selectedMonth}</p>
                  <p className="text-xs sm:text-sm mt-1">Add some expenses to see the breakdown</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className={`rounded-xl sm:rounded-2xl shadow-xl border p-4 sm:p-6 lg:p-8 backdrop-blur-sm ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {categoryData.map((category, index) => (
              <div key={index} className={`flex items-center justify-between p-3 sm:p-4 rounded-lg sm:rounded-xl hover:shadow-lg transition-all duration-200 hover:scale-105 ${
                isDark
